...
const AccountAluno = ({ route, navigation }) => {
    ...
    const takePhoto = async (tipo) => {
        navigation.navigate('CameraScreen', { referencia: 'AccountAluno', tipo: tipo });
    }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (selectedImage === Image.resolveAssetSource(avatarDefault).uri || 
                selectedImage === config.dir_fotos + originalImage) {
                //delete dadosParaSalvar.avatar
                console.log("Mantendo avatar original",dadosParaSalvar.avatar)
            } else {
                console.log("Novo avatar será salvo:", dadosParaSalvar.avatar)
            }
            
            const salvar = await updtCadastro(dadosParaSalvar);
            console.log("Dados enviados para salvar:", dadosParaSalvar)
        ...
        <SafeAreaView style={styles.safeContainer}>
            ...
            <View style={styles.lineBottom}>
                ...
                <TouchableOpacity
                    ...
                    <Ionicons name={'person'} size={30} style={{ color: '#CCC', }} />
                </TouchableOpacity>
                <TouchableOpacity
                    style={styles.btnBottom}
                    onPress={() => signOut()}>
